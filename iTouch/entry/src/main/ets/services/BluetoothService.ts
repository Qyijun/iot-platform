...
class BluetoothService {
  ...
  async requestPermissions(): Promise<boolean> {
    ...
    try {
      ...
      // 检查位置权限
      const locationResult = await this.atManager.checkAccessToken(
        tokenId,
        'ohos.permission.LOCATION'
      );

      // 检查蓝牙权限
      const bluetoothResult = await this.atManager.checkAccessToken(
        tokenId,
      ...
      if (locationResult !== Granted || bluetoothResult !== Granted) {
        ...
        await new Promise<void>((resolve, reject) => {
          this.atManager.requestPermissionsFromUser(
            this.context,
            ['ohos.permission.LOCATION', 'ohos.permission.ACCESS_BLUETOOTH'],
            (err, data) => {
              if (err) {
                reject(err);
